...
| `--all-interfaces` | `false` | Collect every network interface with all of its addresses, name, type, state and MAC into `host.interfaces`. The default `ipv4`/`ipv6`/`macs` stay the usable identity: non-loopback addresses of interfaces that are up. Local enumeration only — no network traffic, and `--root` does not redirect it. |
