...
# Install once with: go install github.com/goreleaser/nfpm/v2/cmd/nfpm@latest
NFPM ?= $(shell command -v nfpm 2>/dev/null || echo "$(GO) run github.com/goreleaser/nfpm/v2/cmd/nfpm@latest")
...
licenses: $(LICENSE_CSV) license-check
	...
	@{ \
	    ...
	    echo; \
	    echo "swinv is Apache-2.0. No dependency below is GPL, AGPL, LGPL, or of"; \
	    echo "unknown licence; \`make license-check\` enforces that in CI."; \
	    echo; \
	    echo "| Module | Licence | Source |"; \
	    echo "|---|---|---|"; \
	    sort -t, -k1,1 $(LICENSE_CSV) | awk -F, 'NF { url=$$2; if (url == "") url="-"; else url="[link](" url ")"; \
	        printf "| `%s` | %s | %s |\n", $$1, $$3, url }'; \
	} > $(LICENSE_DOC)
